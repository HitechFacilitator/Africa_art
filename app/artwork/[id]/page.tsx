      ...
      <main className="flex-1 max-w-[1440px] mx-auto w-full px-6 md:px-16 py-10 md:py-16">
          ...
          <div className="lg:col-span-7 flex flex-col justify-between">
            ...
            <div className={`mt-8 bg-surface p-6 lg:p-8 border-t-2 border-gold-leaf ${accentColors[artwork.id] || "border-l-4 border-[#B35C44]"}`}>
              <div className="flex items-center space-x-3 mb-4">
                ...
                <h3 className="font-serif text-[20px] font-bold text-ebony-deep">Provenance &amp; Authenticity Trace</h3>
              </div>
              ...
              <div className="mt-6 pt-5 border-t border-ebony-deep/10 flex flex-wrap justify-between items-center gap-3">
                <button onClick={() => setShowDossierModal(true)} className="group inline-flex items-center font-sans text-xs font-semibold text-ebony-deep uppercase tracking-wider hover:text-gold-leaf transition-colors">
                  <span className="border-b border-gold-leaf pb-0.5">Request Provenance Dossier</span>
          ...
          <div className="lg:col-span-5 flex flex-col justify-between pt-4 lg:pt-0">
            <div className="space-y-5">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-[10px] tracking-widest uppercase font-bold text-gold-leaf">Investment Grade</span>
              </div>
              ...
              <div className="border-y border-ebony-deep/10 py-5 my-5 space-y-3.5 font-sans text-[13px] text-charcoal-text">
                <div className="flex justify-between border-b border-ebony-deep/5 pb-1.5 font-medium">
                  <span className="text-on-surface-variant font-normal">Origin / Culture</span>
                  <span className="text-right">{artwork.origin} ({artwork.tribe})</span>
                </div>
                <div className="flex justify-between border-b border-ebony-deep/5 pb-1.5 font-medium">
                  <span className="text-on-surface-variant font-normal">Estimated Period</span>
                  <span className="text-right">{artwork.period}</span>
                </div>
                <div className="flex justify-between border-b border-ebony-deep/5 pb-1.5 font-medium">
                  <span className="text-on-surface-variant font-normal">Artistic Medium</span>
                  <span className="text-right">{artwork.material}</span>
                </div>
                <div className="flex justify-between pb-0.5 font-medium">
                  <span className="text-on-surface-variant font-normal">Exhibition Dimensions</span>
              ...
              <div className="mt-8 border border-ebony-deep/15 bg-surface p-[18px]">
                <div className="flex items-center space-x-2 text-ebony-deep border-b border-ebony-deep/10 pb-3 mb-4">
                  ...
                  <div>
                    <h4 className="font-serif text-sm font-bold text-ebony-deep">Consult Curator Vance (AI Support)</h4>
                    <p className="text-[10px] text-on-surface-variant font-sans">Grounding live answers on historical context &amp; science</p>
                  </div>
                </div>
                <div className="space-y-3 h-48 overflow-y-auto mb-4 border border-ebony-deep/5 bg-surface-container-low/50 p-2.5">
                  {chatHistory.length === 0 ? (
                    <div className="text-center h-full flex flex-col items-center justify-center text-xs text-on-surface-variant p-4 font-sans">
                      ...
                      <p className="font-bold">Ask about cultural symbolism or science</p>
                      <p className="text-[10px] mt-0.5">Curator will respond using deep archive records.</p>
                    </div>
                  ) : (
                    chatHistory.map((m, i) => (
                      <div key={i} className={`flex flex-col text-xs leading-relaxed max-w-[90%] ${m.sender === "user" ? "ml-auto items-end" : "mr-auto items-start"}`}>
                        <span className="text-[9px] text-on-surface-variant uppercase tracking-widest font-bold mb-0.5">{m.sender === "user" ? "Advisor Client" : "Lead Curator Vance"}</span>
                  ...
                  {curatorLoading && (
                    <div className="flex items-center space-x-1.5 text-xs text-on-surface-variant p-1.5 font-sans">
                      ...
                      <span className="text-[10px] italic">Reviewing archive logs...</span>
                    </div>
                  )}
                </div>
                <div className="mb-3.5 flex flex-wrap gap-1.5">
                  <button onClick={() => handleQuickQuestion("What is the historical meaning and use of the markings?")} className="text-[10px] bg-surface-container-high hover:bg-surface-container-high/80 text-ebony-deep px-3 py-2 uppercase tracking-wide font-medium">Meaning of markings?</button>
                  <button onClick={() => handleQuickQuestion("How is the age of this piece scientifically verified?")} className="text-[10px] bg-surface-container-high hover:bg-surface-container-high/80 text-ebony-deep px-3 py-2 uppercase tracking-wide font-medium">Scientific age details?</button>
                  <button onClick={() => handleQuickQuestion("Can you detail the lost-wax casting technique?")} className="text-[10px] bg-surface-container-high hover:bg-surface-container-high/80 text-ebony-deep px-3 py-2 uppercase tracking-wide font-medium">Lost-wax casting process?</button>
                </div>
                <form onSubmit={askCurator} className="flex space-x-2">
                  <input type="text" value={curatorQuestion} onChange={(e) => setCuratorQuestion(e.target.value)} placeholder="Inquire Vance regarding this workpiece..." className="flex-grow bg-surface-container-high text-xs px-3 py-2.5 border-b border-ebony-deep/20 focus:border-gold-leaf focus:outline-none placeholder-on-surface-variant font-sans" />
        ...
        <section className="mb-20">
          <div className="text-center max-w-2xl mx-auto mb-12">
            <span className="text-[10px] tracking-[0.25em] font-bold text-gold-leaf uppercase block mb-2 font-sans">Deep Academic Grounding</span>
            <h2 className="font-serif text-[30px] md:text-[34px] lg:text-[38px] text-ebony-deep font-medium leading-tight">Curatorial Insights &amp; Scientific Dossier</h2>
          ...
          <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <motion.article variants={fadeUp} className="bg-surface-container-low p-5 md:p-8 h-full flex flex-col justify-between border-t border-ebony-deep/5">
              <div>
                ...
                <h3 className="font-serif text-[22px] font-bold text-ebony-deep mb-4">Historical Narrative</h3>
            ...
            <motion.article variants={fadeUp} className="bg-surface-container-low p-5 md:p-8 h-full flex flex-col justify-between border-t border-ebony-deep/5">
              <div>
                ...
                <h3 className="font-serif text-[22px] font-bold text-ebony-deep mb-4">The Artist&apos;s Hand</h3>
            ...
            <motion.article variants={fadeUp} className="bg-surface-container-low p-5 md:p-8 h-full flex flex-col justify-between border-t border-ebony-deep/5">
              <div>
                ...
                <h3 className="font-serif text-[22px] font-bold text-ebony-deep mb-4">Scientific Analysis</h3>
          ...
          <div className="fixed inset-0 bg-ebony-deep/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-parchment-ivory border border-gold-leaf max-w-lg w-full p-8 text-ebony-deep shadow-2xl relative">
              ...
              <div className="text-center mb-6">
                ...
                <h3 className="font-serif text-xl font-medium uppercase tracking-wide text-ebony-deep">Provenance Dossier Request</h3>
              </div>
              <div className="bg-surface-container-low border border-ebony-deep/5 p-4 mb-6 text-xs space-y-2">
                ...
                <div className="flex justify-between"><span className="text-on-surface-variant">Medium</span><span className="font-medium">{artwork.material}</span></div>
                <div className="flex justify-between pt-2 border-t border-ebony-deep/5"><span className="text-on-surface-variant">Dossier Status</span><span className="text-gold-leaf font-semibold">Available Upon Request</span></div>
              </div>
