        ...
        <section className="bg-ebony-deep py-12 md:py-16 relative overflow-hidden">
          ...
          <div className="max-w-[1440px] mx-auto px-6 md:px-16 xl:px-20 relative z-10">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="label-caps text-gold-leaf">VIP Exclusive</span>
            </div>
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
              <h1 className="font-display-lg text-parchment-ivory mb-4">Exclusive Previews</h1>
            </motion.div>
            <p className="font-sans text-sm text-parchment-ivory/60 max-w-xl">
              Priority access to upcoming acquisitions before public listing. Express interest
              or reserve artworks directly from this exclusive preview room.
            </p>
            <div className="flex items-center gap-6 mt-6">
              <div className="flex items-center gap-2 text-[10px] uppercase tracking-widest font-semibold text-parchment-ivory/40">
                <Lock size={12} className="text-gold-leaf/70" /> Pre-Publication Access
              </div>
              <div className="flex items-center gap-2 text-[10px] uppercase tracking-widest font-semibold text-parchment-ivory/40">
                <ShieldCheck size={12} className="text-gold-leaf/70" /> VIP Members Only
              </div>
            </div>
          </div>
        </section>
        ...
        <div className="max-w-[1440px] mx-auto px-6 md:px-16 xl:px-20 py-12 md:py-16">
          <div className="flex items-center justify-between mb-8">
            <p className="text-xs text-on-surface-variant">
              <span className="font-bold text-ebony-deep">{UPCOMING_ARTWORKS.length}</span> upcoming acquisitions
            </p>
          </div>
              ...
              >
                <div className="relative bg-ebony-deep overflow-hidden mb-4 aspect-[4/5]">
                  ...
                  <div className="absolute top-3 left-3 bg-ebony-deep/80 backdrop-blur-sm px-2.5 py-1 flex items-center gap-1.5 border border-gold-leaf/20">
                    ...
                    <span className="text-[9px] text-gold-leaf font-bold uppercase tracking-widest">Preview</span>
                  </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-[9px] uppercase tracking-widest text-on-surface-variant font-bold">Estimated Value</p>
                      <p className="font-serif text-sm text-ebony-deep font-semibold">{artwork.estimatedValue}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-[9px] uppercase tracking-widest text-on-surface-variant font-bold">Scarcity</p>
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-parchment-ivory max-w-lg w-full p-8 text-ebony-deep shadow-2xl relative max-h-[90vh] overflow-y-auto">
              ...
              {porSubmitted ? (
                <div className="text-center">
                  ...
                  <h3 className="font-serif text-xl font-medium uppercase tracking-wide mb-3">Priority Inquiry Received</h3>
                  <p className="text-xs text-on-surface-variant mb-6 leading-relaxed">
                    Your VIP priority inquiry for <strong>{selectedArtwork.title}</strong> has been received.
                    As a priority member, our curatorial team will respond within 12 hours with full pricing and provenance documentation.
                  </p>
                  <button onClick={resetPOR} className="bg-ebony-deep text-parchment-ivory px-8 py-3 text-xs uppercase tracking-widest font-bold hover:opacity-90 transition-opacity cursor-pointer border-0">Close</button>
                </div>
              ) : (
                <>
                  <div className="text-center mb-6">
                    <Sparkles className="w-10 h-10 text-gold-leaf mx-auto mb-3" />
                    <h3 className="font-serif text-xl font-medium uppercase tracking-wide">VIP Priority Inquiry</h3>
                    <p className="text-xs text-on-surface-variant mt-1">For: {selectedArtwork.title}</p>
                    <p className="text-[10px] text-gold-leaf mt-1 font-semibold">Priority Response within 12 Hours</p>
                  </div>
                  <form onSubmit={(e) => { e.preventDefault(); if (!porFirstName || !porLastName || !porEmail || !gdprConsent) return; setPorSubmitted(true); }} className="space-y-4">
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-ebony-deep/5">
                      <button type="button" onClick={resetPOR} className="border border-ebony-deep/20 px-6 py-2.5 text-xs font-bold uppercase tracking-widest text-zinc-500 hover:text-ebony-deep cursor-pointer bg-transparent">Cancel</button>
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-parchment-ivory max-w-lg w-full p-8 text-ebony-deep shadow-2xl relative">
              ...
              {reserveConfirmed ? (
                <div className="text-center">
                  ...
                  <p className="text-xs text-on-surface-variant mb-6 leading-relaxed">
                    <strong>{selectedArtwork.title}</strong> has been reserved in your name for 48 hours.
                    A confirmation email has been sent with your reservation details.
                  </p>
                  ...
                  <div className="flex gap-3 justify-center">
                    <a href="/dashboard" className="bg-ebony-deep text-parchment-ivory px-6 py-3 text-xs uppercase tracking-widest font-bold hover:bg-gold-leaf hover:text-ebony-deep transition-colors inline-block">Dashboard</a>
                    <a href="/acquisition" className="border border-gold-leaf text-gold-leaf px-6 py-3 text-xs uppercase tracking-widest font-bold hover:bg-gold-leaf/10 transition-colors inline-block">Purchase</a>
                  </div>
                </div>
              ) : (
                <>
                  <div className="text-center mb-6">
                    ...
                    <h3 className="font-serif text-xl font-medium uppercase tracking-wide">Reserve for 48 Hours</h3>
                    <p className="text-xs text-gold-leaf font-semibold mt-1">VIP Immediate Reservation</p>
                  </div>
                  <div className="bg-surface-container-low border border-ebony-deep/5 p-4 mb-6 text-xs space-y-2">
                    <div className="flex justify-between"><span className="text-on-surface-variant">Artwork</span><span className="font-medium">{selectedArtwork.title}</span></div>
                    <div className="flex justify-between"><span className="text-on-surface-variant">Origin</span><span className="font-medium">{selectedArtwork.origin}</span></div>
                    <div className="flex justify-between"><span className="text-on-surface-variant">Reservation</span><span className="font-medium text-gold-leaf font-semibold">48 Hours</span></div>
                  </div>
                  <p className="text-xs text-on-surface-variant mb-6 leading-relaxed">
                    As a VIP member, you have immediate 48-hour reservation priority. The artwork will be locked from public sale during this period.
                  </p>
                  <div className="flex gap-3">
                    <button onClick={resetReserve} className="flex-1 border border-ebony-deep/20 px-6 py-3 text-xs font-bold uppercase tracking-widest text-zinc-500 hover:text-ebony-deep cursor-pointer bg-transparent">Cancel</button>
                    <button
