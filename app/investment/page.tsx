...
export default function InvestmentPage() {
  ...
  const [messages, setMessages] = useState<{ sender: "user" | "coordinator"; text: string; time: string }[]>([{ sender: "coordinator", text: "Connection secure: AES-256 Tunnel established. Good day, collector. We stand ready to coordinate Swiss freeport handovers or verify pending London bids.", time: "21:19:15" }]);
  ...
  const handleAuth = (e: React.FormEvent) => {
    ...
    if (accessCode.trim().toLowerCase() === "vault" || accessCode.trim() === "1234" || accessCode.trim().length > 2) {
      ...
      setAuthError("Session authentication failed: Invalid collector security hash.");
    }
  };
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      let reply = "Your request has been logged. Under our Swiss sovereign privacy regulations, I have queued this inquiry for Dr. Elena Rostova's direct offline evaluation.";
      if (textLower.includes("nok") || textLower.includes("terracotta")) reply = "Acknowledged. Our Nok Seated Dignitary is currently secured in Zurich Chamber 14B. Direct inspection requires 48-hour prior clearing notice.";
      else if (textLower.includes("benin") || textLower.includes("bronze")) reply = "Acknowledged. The Benin Consortium includes two relief plaques currently on escrow holding status in Geneva.";
      else if (textLower.includes("escrow") || textLower.includes("buy")) reply = "Under our Escrow protocol, buy-orders are routed via deep-blind custody accounts with Julius Baer Group.";
          ...
          {activeTab === "advisory" && (
            <motion.div key="advisory" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.4 }}>
              <section className="mb-24 flex flex-col items-center text-center max-w-4xl mx-auto px-4 pt-10">
                <span className="font-sans text-xs uppercase tracking-[0.2em] text-gold-leaf font-bold mb-6">Institutional Advisory Services</span>
                <h1 className="font-display-xl text-ebony-deep mb-8 leading-[1.1]">Curated Portfolios of Cultural Permanence</h1>
                <p className="font-serif text-lg md:text-xl text-on-surface-variant mb-12 max-w-2xl leading-relaxed">Strategic acquisition and divestment counsel for high-net-worth collectors and institutions.</p>
                <div className="flex flex-col sm:flex-row gap-4">
                  <button onClick={() => formRef.current?.scrollIntoView({ behavior: "smooth" })} className="bg-ebony-deep text-parchment-ivory font-sans text-xs uppercase tracking-widest px-8 py-5 hover:bg-gold-leaf hover:text-ebony-deep transition-colors">Schedule Consultation</button>
                  <button onClick={() => setActiveTab("provenance")} className="border border-gold-leaf text-gold-leaf font-sans text-xs uppercase tracking-widest px-8 py-5 hover:bg-gold-leaf/5 transition-colors bg-transparent">Verify Provenance Files</button>
                </div>
              </section>
              ...
              <section className="mb-24 max-w-7xl mx-auto px-6 md:px-12">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-4">
                  <div>
                    <span className="font-sans text-xs uppercase tracking-widest text-gold-leaf font-bold">Consolidated Index Analytics</span>
                    <h2 className="font-serif text-3xl md:text-4xl text-ebony-deep mt-2">Market Performance: African Antiquities</h2>
                  </div>
                  <div className="flex gap-2 bg-surface-container/60 p-1 border border-on-surface/5 uppercase text-[10px] tracking-wider font-semibold">
                    {(["aggregate", "nok", "benin"] as const).map((f) => (
                      <button key={f} onClick={() => setChartFocus(f)} className={`px-3 py-1.5 transition-all ${chartFocus === f ? "bg-ebony-deep text-parchment-ivory" : "text-on-surface-variant/70 hover:text-ebony-deep"}`}>
                        {f === "aggregate" ? "Aggregate Index" : f === "nok" ? "Nok Terracotta" : "Benin Bronze"}
                      </button>
                    ))}
                  </div>
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  <div className="lg:col-span-8 bg-surface-container-lowest p-6 md:p-8 border border-on-surface/5 shadow-level-2">
                    <div className="flex justify-between items-start mb-8">
                      <div>
                        <span className="font-sans text-[11px] uppercase tracking-wider text-on-surface-variant/60">INDEX VALUATION BASELINE (2016 = 100)</span>
                        <h3 className="font-serif text-xl md:text-2xl text-ebony-deep mt-1">{chartFocus === "aggregate" ? "Historical Growth Pattern" : chartFocus === "nok" ? "Nok Terracotta Investment Wave" : "Kingdom of Benin Bronze Benchmark"}</h3>
                      </div>
                      <div className="text-right">
                        ...
                        <span className="text-[10px] uppercase font-bold text-on-surface-variant/50">SECURE TRANSACTION VOLUME</span>
                      </div>
                    </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-on-surface/5 flex flex-col sm:flex-row justify-between text-[11px] text-on-surface-variant/60 font-sans gap-2">
                      <span>* Data verified according to Swiss and London auction house indices</span>
                      <span className="font-medium text-gold-leaf uppercase tracking-wider">● Interactive Valuation View Enabled</span>
                    </div>
                  </div>
                  <div className="lg:col-span-4 flex flex-col gap-6">
                    <div className="bg-surface-container-lowest p-6 border border-on-surface/5 shadow-level-2 flex-1">
                      ...
                      <span className="font-sans text-xs uppercase tracking-widest text-on-surface-variant/60 block mb-1">Global Auction Volume</span>
                      <div className="font-serif text-3xl md:text-4xl text-ebony-deep font-semibold">$124.5M</div>
                      <div className="mt-4 pt-4 border-t border-on-surface/5 text-xs text-on-surface-variant/60 flex items-center justify-between"><span>2023 Fiscal Year</span><span className="text-[10px] uppercase font-bold text-emerald-600">+9.4% YoY</span></div>
                    </div>
                    <div className="bg-surface-container-lowest p-6 border-l-4 border-terracotta-earth shadow-level-2 flex-1">
                      ...
                      <span className="font-sans text-xs uppercase tracking-widest text-on-surface-variant/60 block mb-1">10-Year Appreciation</span>
                      <div className="font-serif text-3xl md:text-4xl text-ebony-deep font-semibold">218%</div>
                      <div className="mt-4 pt-4 border-t border-on-surface/5 text-xs text-on-surface-variant/60 flex items-center justify-between"><span>Top-Tier Provenance Only</span><span className="text-[10px] uppercase font-bold text-terracotta-earth">Compounded</span></div>
                    </div>
                  </div>
                </div>
              </section>
              ...
              <section className="mb-24 bg-surface-container-low/40 py-20 px-6 md:px-12 border-y border-on-surface/5">
                <div className="max-w-7xl mx-auto">
                  <div className="text-center mb-16">
                    <span className="font-sans text-xs uppercase tracking-[0.2em] text-gold-leaf font-bold block mb-3">Sovereign Asset Holdings</span>
                    <h2 className="font-serif text-3xl md:text-4xl text-ebony-deep mb-4">Curated Investment Packages</h2>
                  </div>
                      ...
                      <div key={pkg.id} onClick={() => setSelectedPackage(pkg)} className="group cursor-pointer flex flex-col bg-surface-container-lowest shadow-level-2 border border-on-surface/5 hover:border-gold-leaf/20 transition-all duration-300">
                        ...
                        <div className="p-8 text-center flex-1 flex flex-col justify-between">
                          ...
                          <div className="pt-6 border-t border-on-surface/5 flex justify-center">
                            <button className="font-sans text-xs uppercase tracking-widest text-ebony-deep border-b border-gold-leaf pb-1 hover:text-gold-leaf transition-colors">View Portfolio Breakdown</button>
                          </div>
                        </div>
                      </div>
                    ))}
              ...
              <section ref={formRef} className="mb-24 max-w-7xl mx-auto px-6 md:px-12 scroll-mt-24">
                <div className="bg-surface-container-lowest border border-on-surface/5 shadow-level-2 p-5 md:p-8 lg:p-16 flex flex-col lg:flex-row gap-16 items-center">
                  <div className="w-full lg:w-1/2">
                    <span className="font-sans text-xs uppercase tracking-widest text-gold-leaf font-bold block mb-4">CONFIDENTIAL COMMUNIQUE</span>
                    <h2 className="font-serif text-3xl md:text-4xl text-ebony-deep mb-6">Confidential Advisory</h2>
                    <p className="font-serif text-base text-on-surface-variant mb-8 leading-relaxed">Engage directly with our senior curators, legal provenance specialists, and financial analysts.</p>
                    <div className="border-l-4 border-terracotta-earth pl-6 mb-8 py-2 bg-surface-container-low/30 pr-4">
                      <p className="font-serif text-sm text-ebony-deep italic leading-relaxed">&quot;True value in antiquities is derived not just from aesthetic raw scarcity, but from the unassailable clarity of its historical provenance.&quot;</p>
                      <span className="font-sans text-[10px] uppercase tracking-wider text-on-surface-variant/60 mt-4 block font-bold">— Dr. Elena Rostova, Head of Advisory Board</span>
                    </div>
                    <div className="flex gap-4 text-xs text-on-surface-variant/60 font-sans">
                      <div className="flex items-center gap-1.5 border border-on-surface/10 px-3 py-1.5 bg-surface-container-low"><ShieldCheck className="w-4 h-4 text-gold-leaf" /><span>NDA Protected</span></div>
                      <div className="flex items-center gap-1.5 border border-on-surface/10 px-3 py-1.5 bg-surface-container-low"><UserCheck className="w-4 h-4 text-gold-leaf" /><span>Biometric Validation</span></div>
                    </div>
                  </div>
                    ...
                    <AnimatePresence mode="wait">
                      {!bookingSuccess ? (
                        <motion.form key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onSubmit={handleBooking} className="flex flex-col gap-6">
                          <div className="flex flex-col"><label className="font-sans text-[10px] uppercase tracking-wider text-on-surface-variant/60 mb-2 font-bold">Inquiry Type</label><select value={inquiryType} onChange={(e) => setInquiryType(e.target.value)} className="bg-transparent border-0 border-b border-on-surface/20 focus:ring-0 focus:border-gold-leaf py-2 px-0 font-sans text-sm text-ebony-deep outline-none cursor-pointer"><option>Portfolio Assessment & Allocation</option><option>Direct Acquisition Strategy</option><option>Authentication & Carbon Vetting</option><option>Secure Escrow Storage Setup</option></select></div>
                          <div className="flex flex-col"><label className="font-sans text-[10px] uppercase tracking-wider text-on-surface-variant/60 mb-2 font-bold">Full Name</label><input type="text" required placeholder="Baron Robert de Rothschild" value={fullName} onChange={(e) => setFullName(e.target.value)} className="bg-transparent border-0 border-b border-on-surface/20 focus:ring-0 focus:border-gold-leaf py-2 px-0 font-sans text-sm text-ebony-deep outline-none placeholder:text-on-surface-variant/30" /></div>
                          <div className="flex flex-col"><label className="font-sans text-[10px] uppercase tracking-wider text-on-surface-variant/60 mb-2 font-bold">Encrypted Contact Email</label><input type="email" required placeholder="r.rothschild@sovereign-vault.ch" value={email} onChange={(e) => setEmail(e.target.value)} className="bg-transparent border-0 border-b border-on-surface/20 focus:ring-0 focus:border-gold-leaf py-2 px-0 font-sans text-sm text-ebony-deep outline-none placeholder:text-on-surface-variant/30" /></div>
                           <div className="flex flex-col"><label className="font-sans text-[10px] uppercase tracking-wider text-on-surface-variant/60 mb-2 font-bold">Preferred Advisory Curator</label><select value={preferredCurator} onChange={(e) => setPreferredCurator(e.target.value)} className="bg-transparent border-0 border-b border-on-surface/20 focus:ring-0 focus:border-gold-leaf py-2 px-0 font-sans text-sm text-ebony-deep outline-none cursor-pointer"><option>Dr. Elena Rostova (African Antiquities)</option><option>Marcus Vance (Financial Art Allocation)</option><option>Consensus Council (Full Advisory Board)</option></select></div>
                           <div className="flex flex-col"><label className="font-sans text-[10px] uppercase tracking-wider text-on-surface-variant/60 mb-2 font-bold">Primary Allocation Target</label><select value={investmentTier} onChange={(e) => setInvestmentTier(e.target.value)} className="bg-transparent border-0 border-b border-on-surface/20 focus:ring-0 focus:border-gold-leaf py-2 px-0 font-sans text-sm text-ebony-deep outline-none cursor-pointer"><option>$250,000 - $500,000</option><option>$500,000 - $2,000,000</option><option>$2,000,000 - $5,000,000</option><option>$5,000,000+ (Institutional Mandate)</option></select></div>
                           <div className="flex flex-col"><label className="font-sans text-[10px] uppercase tracking-wider text-on-surface-variant/60 mb-2 font-bold">Investment Horizon</label><select value={investmentHorizon} onChange={(e) => setInvestmentHorizon(e.target.value)} className="bg-transparent border-0 border-b border-on-surface/20 focus:ring-0 focus:border-gold-leaf py-2 px-0 font-sans text-sm text-ebony-deep outline-none cursor-pointer"><option>1-2 Years</option><option>3-5 Years</option><option>5-10 Years</option><option>10+ Years (Legacy)</option></select></div>
                           <div className="flex flex-col"><label className="font-sans text-[10px] uppercase tracking-wider text-on-surface-variant/60 mb-2 font-bold">Investment Goals</label><select value={investmentGoals} onChange={(e) => setInvestmentGoals(e.target.value)} className="bg-transparent border-0 border-b border-on-surface/20 focus:ring-0 focus:border-gold-leaf py-2 px-0 font-sans text-sm text-ebony-deep outline-none cursor-pointer"><option>Capital Appreciation</option><option>Diversification</option><option>Cultural Preservation</option><option>Tax Optimization</option><option>Portfolio Hedging</option></select></div>
                           <div className="flex flex-col"><label className="font-sans text-[10px] uppercase tracking-wider text-on-surface-variant/60 mb-2 font-bold">Current Collection (Optional)</label><input type="text" placeholder="Describe your existing collection..." value={currentCollection} onChange={(e) => setCurrentCollection(e.target.value)} className="bg-transparent border-0 border-b border-on-surface/20 focus:ring-0 focus:border-gold-leaf py-2 px-0 font-sans text-sm text-ebony-deep outline-none placeholder:text-on-surface-variant/30" /></div>
                           <div className="border-t border-on-surface/10 pt-4 mt-2">
                             <p className="font-sans text-[10px] uppercase tracking-wider text-terracotta-earth mb-3 font-bold">Schedule Valuation Meeting</p>
                             <div className="grid grid-cols-2 gap-4">
                               <div className="flex flex-col"><label className="font-sans text-[10px] uppercase tracking-wider text-on-surface-variant/60 mb-2 font-bold">Preferred Date</label><input type="date" value={preferredDate} onChange={(e) => setPreferredDate(e.target.value)} className="bg-transparent border-0 border-b border-on-surface/20 focus:ring-0 focus:border-gold-leaf py-2 px-0 font-sans text-sm text-ebony-deep outline-none" /></div>
                               <div className="flex flex-col"><label className="font-sans text-[10px] uppercase tracking-wider text-on-surface-variant/60 mb-2 font-bold">Preferred Time</label><select value={preferredTime} onChange={(e) => setPreferredTime(e.target.value)} className="bg-transparent border-0 border-b border-on-surface/20 focus:ring-0 focus:border-gold-leaf py-2 px-0 font-sans text-sm text-ebony-deep outline-none cursor-pointer"><option>Morning (9:00-12:00 CET)</option><option>Afternoon (13:00-17:00 CET)</option><option>Evening (18:00-20:00 CET)</option></select></div>
                             </div>
                             <div className="flex flex-col mt-4"><label className="font-sans text-[10px] uppercase tracking-wider text-on-surface-variant/60 mb-2 font-bold">Meeting Format</label><select value={meetingType} onChange={(e) => setMeetingType(e.target.value)} className="bg-transparent border-0 border-b border-on-surface/20 focus:ring-0 focus:border-gold-leaf py-2 px-0 font-sans text-sm text-ebony-deep outline-none cursor-pointer"><option>Video Call (Secure Zoom)</option><option>In-Person (Zurich)</option><option>In-Person (London)</option><option>Phone Consultation</option></select></div>
                           </div>
                          <button type="submit" className="mt-6 bg-ebony-deep text-parchment-ivory font-sans text-xs uppercase tracking-widest px-6 py-4 hover:bg-gold-leaf hover:text-ebony-deep transition-colors">Request Secure Appointment</button>
                        </motion.form>
                      ) : (
                          ...
                          <h3 className="font-serif text-2xl text-ebony-deep mb-4">Secure Connection Established</h3>
                          <p className="font-serif text-sm text-on-surface-variant mb-6 max-w-sm leading-relaxed">Thank you, <strong className="text-ebony-deep">{fullName}</strong>. Your advisory request for <strong className="text-ebony-deep">{inquiryType}</strong> has been logged.</p>
                          <div className="bg-surface-container p-4 border border-on-surface/5 mb-8 w-full text-left">
                            <div className="space-y-1.5 text-xs font-sans">
                              <div className="flex justify-between"><span className="text-on-surface-variant/50">Vetting Code:</span><span className="font-bold text-ebony-deep font-mono">{bookingRef}</span></div>
                              <div className="flex justify-between"><span className="text-on-surface-variant/50">Assigned Curator:</span><span className="text-ebony-deep font-medium">{preferredCurator}</span></div>
                              <div className="flex justify-between"><span className="text-on-surface-variant/50">Target Volume:</span><span className="text-ebony-deep font-medium">{investmentTier}</span></div>
                              <div className="flex justify-between"><span className="text-on-surface-variant/50">Horizon:</span><span className="text-ebony-deep font-medium">{investmentHorizon}</span></div>
                              <div className="flex justify-between"><span className="text-on-surface-variant/50">Goals:</span><span className="text-ebony-deep font-medium">{investmentGoals}</span></div>
                              {preferredDate && <div className="flex justify-between"><span className="text-on-surface-variant/50">Meeting:</span><span className="text-ebony-deep font-medium">{preferredDate} · {preferredTime}</span></div>}
                              {meetingType && <div className="flex justify-between"><span className="text-on-surface-variant/50">Format:</span><span className="text-ebony-deep font-medium">{meetingType}</span></div>}
                            </div>
                          </div>
                          <button onClick={() => { setBookingSuccess(false); setFullName(""); setEmail(""); setCurrentCollection(""); setPreferredDate(""); }} className="border border-on-surface/10 font-sans text-xs uppercase tracking-widest px-6 py-3 hover:bg-on-surface/5 transition-colors">Authorize Another Request</button>
                        </motion.div>
                      )}
                    </AnimatePresence>
                  </div>
            ...
            <motion.div key="gallery" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.4 }}>
              <div className="bg-parchment-ivory min-h-screen py-12 md:py-20">
                <div className="max-w-7xl mx-auto px-6 md:px-12">
                  <div className="text-center mb-16">
                    <span className="font-sans text-xs uppercase tracking-[0.2em] text-gold-leaf font-bold block mb-3">CONSECUTIVE ASSET VAULT</span>
                    <h1 className="font-serif text-4xl md:text-5xl text-ebony-deep mb-4">The Sovereign Antiquities Gallery</h1>
                    <p className="font-serif text-base text-on-surface-variant max-w-xl mx-auto">Explore a refined catalog of verified physical artifacts cleared by archaeological advisors.</p>
                  </div>
                  ...
                  <div className="bg-surface-container/60 p-6 border border-on-surface/5 mb-12 flex flex-col md:flex-row gap-6 justify-between items-stretch md:items-center">
                    <div className="relative flex-1 max-w-md">
                      ...
                      <input type="text" placeholder="Search via civilization..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-parchment-ivory border border-on-surface/15 py-2.5 pl-10 pr-4 font-sans text-xs focus:outline-none focus:border-gold-leaf text-ebony-deep placeholder:text-on-surface-variant/40" />
                    </div>
                    <div className="flex flex-wrap items-center gap-4 text-xs font-sans">
                      <div className="flex items-center gap-2"><span className="text-on-surface-variant/60 font-bold uppercase tracking-wider text-[10px]">Culture:</span><select value={regionFilter} onChange={(e) => setRegionFilter(e.target.value)} className="bg-parchment-ivory border border-on-surface/15 py-2 px-3 focus:outline-none focus:border-gold-leaf text-ebony-deep font-semibold cursor-pointer"><option value="All">All Cultures</option><option value="Nok">Nok Plateau</option><option value="Ife">Ife Kingdom</option><option value="Benin">Kingdom of Benin</option></select></div>
                      <div className="flex items-center gap-2"><span className="text-on-surface-variant/60 font-bold uppercase tracking-wider text-[10px]">Medium:</span><select value={mediumFilter} onChange={(e) => setMediumFilter(e.target.value)} className="bg-parchment-ivory border border-on-surface/15 py-2 px-3 focus:outline-none focus:border-gold-leaf text-ebony-deep font-semibold cursor-pointer"><option value="All">All Mediums</option><option value="Terracotta">Terracotta</option><option value="Bronze">Bronze</option></select></div>
                      {(regionFilter !== "All" || mediumFilter !== "All" || searchQuery !== "") && <button onClick={() => { setRegionFilter("All"); setMediumFilter("All"); setSearchQuery(""); }} className="text-terracotta-earth font-bold hover:underline uppercase tracking-wider text-[10px]">Reset Filter</button>}
                    </div>
                  </div>
                  ...
                  {filteredArtifacts.length > 0 ? (
                    <motion.div variants={stagger} initial="hidden" animate="visible" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 lg:gap-12">
                      {filteredArtifacts.map((art) => (
                        <motion.div variants={fadeUp} key={art.id} onClick={() => setActiveGalleryArtifact(art)} className="group cursor-pointer bg-surface-container-lowest border border-on-surface/5 shadow-level-2 hover:shadow-hover-lift transition-all duration-300 flex flex-col justify-between">
                          ...
                          <div className="bg-ebony-deep text-parchment-ivory/95 text-center text-[10px] uppercase tracking-widest py-3 hover:bg-gold-leaf hover:text-ebony-deep transition-colors">Authenticate &amp; View Dossier</div>
                        </motion.div>
                      ))}
                    </motion.div>
                  ) : (
                    <div className="text-center py-20 bg-surface-container-lowest border border-on-surface/5">
                      ...
                      <h3 className="font-serif text-xl text-ebony-deep">No Matches Registered</h3>
                    </div>
                  )}
                </div>
              </div>
                  ...
                  <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4">
                    ...
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-parchment-ivory border border-on-surface/10 max-w-5xl w-full p-6 md:p-10 relative z-10 shadow-2xl max-h-[90vh] overflow-y-auto">
                      <div className="flex justify-between items-start border-b border-on-surface/15 pb-4 mb-6">
                        <div><span className="font-sans text-[10px] uppercase tracking-widest text-terracotta-earth font-bold block">COLLECTOR ACCESS CONSOLE</span><h3 className="font-serif text-2xl md:text-3xl text-ebony-deep font-semibold mt-1">{activeGalleryArtifact.title}</h3></div>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                        <div className="lg:col-span-7 space-y-6">
                          <div className="relative aspect-video max-h-96 w-full bg-surface-container-low overflow-hidden border border-on-surface/5">
                            ...
                            <div className="absolute bottom-3 left-3 bg-ebony-deep text-parchment-ivory text-[10px] font-sans pb-1 pt-1.5 px-3 uppercase tracking-widest font-semibold">ESTIMATED VALUE: {activeGalleryArtifact.estimatedValue}</div>
                          </div>
                          <div><h4 className="font-sans text-xs uppercase tracking-widest text-gold-leaf font-bold mb-2">Scientific Authentication Report</h4><p className="font-serif text-sm text-on-surface-variant/80 bg-surface-container p-3 border-l-2 border-terracotta-earth leading-relaxed">{activeGalleryArtifact.carbonDatingDetails || "Thermoluminescence testing validates creation epoch."}</p></div>
                          <div><h4 className="font-sans text-xs uppercase tracking-widest text-terracotta-earth font-bold mb-3">Provenance Ledger</h4><div className="space-y-2 font-sans text-xs">{activeGalleryArtifact.provenance.map((prov, i) => <div key={i} className="py-2 border-b border-on-surface/5 text-on-surface-variant/80 pl-3 border-l-2 border-gold-leaf bg-surface-container-low/40">{prov}</div>)}</div></div>
                          <div><h4 className="font-sans text-xs uppercase tracking-widest text-terracotta-earth font-bold mb-2">Exhibition Registry</h4><ul className="space-y-1 text-xs text-on-surface-variant/80">{activeGalleryArtifact.exhibitions.map((ex, i) => <li key={i} className="flex gap-2 items-center"><span className="w-1.5 h-1.5 bg-gold-leaf rounded-full shrink-0" /><span>{ex}</span></li>)}</ul></div>
                        </div>
                        <div className="lg:col-span-5 space-y-6">
                          <div><h4 className="font-sans text-xs uppercase tracking-widest text-ebony-deep font-bold mb-2 border-b border-on-surface/10 pb-2">Curatorial Statement</h4><p className="font-serif text-sm text-on-surface-variant leading-relaxed">{activeGalleryArtifact.description}</p></div>
                          <div className="bg-surface-container p-4 border border-on-surface/5 font-sans space-y-2.5 text-xs">
                            <div className="flex justify-between border-b border-on-surface/10 pb-1.5"><span className="text-on-surface-variant/50">Accession Code</span><span className="font-bold text-ebony-deep font-mono">{activeGalleryArtifact.accessionNo}</span></div>
                            <div className="flex justify-between border-b border-on-surface/10 pb-1.5"><span className="text-on-surface-variant/50">Culture</span><span className="font-semibold text-ebony-deep">{activeGalleryArtifact.origin}</span></div>
                            <div className="flex justify-between"><span className="text-on-surface-variant/50">Medium</span><span className="font-semibold text-ebony-deep">{activeGalleryArtifact.medium}</span></div>
                          </div>
                          <div className="bg-surface-container p-6 border border-gold-leaf/25 relative overflow-hidden">
                            <div className="absolute top-0 right-0 bg-gold-leaf/10 border-l border-b border-gold-leaf/30 text-gold-leaf px-2.5 py-0.5 text-[8.5px] uppercase tracking-widest font-extrabold">CONFIDENTIAL</div>
                            <AnimatePresence mode="wait">
                              {!inquirySuccess ? (
                                ...
                                <motion.div key="inq-success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-6">
                                  ...
                                  <h4 className="font-serif text-sm text-ebony-deep font-bold mb-1">Credentials Lodged Successfully</h4>
                                </motion.div>
                              )}
                            </AnimatePresence>
                          </div>
              ...
              <div className="bg-parchment-ivory min-h-screen py-12 md:py-20">
                <div className="max-w-7xl mx-auto px-6 md:px-12">
                  <div className="text-center mb-16 max-w-2xl mx-auto">
                    <span className="font-sans text-xs uppercase tracking-[0.2em] text-gold-leaf font-bold block mb-3">continuous custody records</span>
                    <h1 className="font-serif text-4xl md:text-5xl text-ebony-deep mb-4">Unassailable Provenance Registry</h1>
                    <p className="font-serif text-base text-on-surface-variant">Every transaction, carbon-dating, and historical archive entry logged.</p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8 lg:gap-12 items-start mb-20">
                    <div className="lg:col-span-4 space-y-6">
                      <div className="bg-surface-container/60 p-6 border border-on-surface/5 shadow-level-2">
                        <h3 className="font-sans text-xs uppercase tracking-widest text-terracotta-earth font-bold mb-4">Accession Key Audit</h3>
                        <form onSubmit={handleSearchCode} className="space-y-4">
                          <div className="relative"><SearchCode className="absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant/40 w-4 h-4" /><input type="text" placeholder="Enter Accession: NOK-9401..." value={typedCode} onChange={(e) => setTypedCode(e.target.value)} className="w-full bg-parchment-ivory border border-on-surface/15 py-3 pl-10 pr-3 font-sans text-xs focus:outline-none focus:border-gold-leaf uppercase" /></div>
                          <button type="submit" className="w-full bg-ebony-deep text-parchment-ivory font-sans text-xs uppercase tracking-widest py-3 hover:bg-gold-leaf hover:text-ebony-deep transition-colors">Query Custody Ledger</button>
                        </form>
                      ...
                      <div className="bg-surface-container/60 p-6 border border-on-surface/5 shadow-level-2">
                        <h3 className="font-sans text-xs uppercase tracking-widest text-ebony-deep font-bold mb-4">Master Vault Items</h3>
                    ...
                    <div className="lg:col-span-8 bg-surface-container-lowest p-6 md:p-10 border border-on-surface/5 shadow-level-2">
                      <div className="flex flex-col sm:flex-row justify-between items-start border-b border-on-surface/10 pb-6 mb-8 gap-4">
                        <div><span className="font-sans text-[10px] uppercase tracking-widest text-gold-leaf font-bold block">Vetted custody timeline</span><h2 className="font-serif text-2xl md:text-3xl text-ebony-deep font-semibold mt-1">{activeProvenanceArt.title}</h2><span className="font-sans text-xs text-on-surface-variant/50">Origin: {activeProvenanceArt.origin}</span></div>
                        <div className="bg-surface-container px-4 py-2 text-right"><span className="font-serif text-terracotta-earth text-lg font-bold block">{activeProvenanceArt.estimatedValue}</span><span className="text-[9px] uppercase tracking-wider font-bold text-on-surface-variant/50">Appraised Valuation</span></div>
                      </div>
                          ...
                          return (
                            <div key={idx} className="relative">
                              ...
                              <div>
                                ...
                                <div className="mt-2 flex gap-4 text-[10px] font-sans text-on-surface-variant/40"><span>● Handover Signed & Hashed</span><span>● Export License Clear</span></div>
                              </div>
                            </div>
                          );
                        })}
                      ...
                      <div className="mt-12 pt-8 border-t border-on-surface/10 grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div><span className="font-sans text-[10px] uppercase tracking-wider text-gold-leaf font-bold block mb-2">THERMOLUMINESCENCE ACCURACY</span><p className="font-serif text-xs text-on-surface-variant/80 leading-relaxed bg-surface-container p-4 border-l-2 border-gold-leaf">{activeProvenanceArt.carbonDatingDetails || "Physical testing verifies thermoluminescence decay matching the certified creation epoch."}</p></div>
                        <div><span className="font-sans text-[10px] uppercase tracking-wider text-gold-leaf font-bold block mb-2">CUSTODY REGULATION ACCLAIM</span><p className="font-serif text-xs text-on-surface-variant/80 leading-relaxed bg-surface-container p-4 border-l-2 border-terracotta-earth">This record complies with article 2(a) of the UNESCO 1970 Convention on transit of native heritage.</p></div>
                      </div>
                    </div>
                  </div>

                  <section className="border-t border-on-surface/10 pt-16">
                    <div className="text-center mb-12"><span className="font-sans text-xs uppercase tracking-[0.2em] text-terracotta-earth font-bold block mb-2">Institutional Authentications</span><h2 className="font-serif text-2xl md:text-3xl text-ebony-deep">Double-Blind Authentication Guidelines</h2></div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                      {SECURITY_AUDITS.map((item, idx) => (
                        <div key={idx} className="bg-surface-container p-6 border-t border-gold-leaf/30 space-y-3">
                          <span className="text-[10px] uppercase font-bold text-on-surface-variant/40">Audit Phase 0{idx + 1}</span>
                          <h4 className="font-serif text-base text-ebony-deep font-semibold">{item.title}</h4>
                          <div className="text-xs"><span className="font-bold text-terracotta-earth block">Authority:</span><span className="text-on-surface-variant/70">{item.authority}</span></div>
                          <div className="text-xs"><span className="font-bold text-terracotta-earth block">Standard:</span><span className="text-on-surface-variant/70 font-mono text-[11px]">{item.standard}</span></div>
                    ...
                    {!isAuthenticated ? (
                      <motion.div key="gateway" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} className="max-w-md mx-auto text-center">
                        <div className="bg-ebony-deep text-gold-leaf p-10 border border-gold-leaf/20 shadow-2xl relative">
                          ...
                          <h2 className="font-serif text-2xl mb-2 mt-4 text-center text-gold-leaf">Encrypted Vault Terminal</h2>
                          <p className="text-[11px] uppercase tracking-widest text-terracotta-earth font-bold mb-6 text-center">Swiss Freeport Ledger Access</p>
                          <p className="text-parchment-ivory/60 text-xs text-center leading-relaxed mb-6 font-serif">Enter your secure Collector Key to query current physical audits and communicate behind our encrypted firewall.</p>
                          <form onSubmit={handleAuth} className="space-y-4">
                            ...
                            <button type="submit" className="w-full bg-gold-leaf text-ebony-deep font-sans text-xs uppercase tracking-widest font-extrabold py-3.5 hover:bg-gold-leaf/80 transition-colors">Authenticate Ledger Session</button>
                          </form>
                          {authError && <div className="mt-4 text-[11px] text-rose-500 font-bold tracking-tight">{authError}</div>}
                          <div className="mt-6 pt-4 border-t border-neutral-800 flex items-center justify-center gap-1.5 text-[10px] text-neutral-500 font-sans"><Radio className="w-3.5 h-3.5 text-terracotta-earth animate-pulse" /><span>SECURE HSM CONNECTION ACTIVE</span></div>
                        </div>
                      </motion.div>
                    ) : (
                      <motion.div key="dashboard" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="space-y-12">
                        <div className="bg-surface-container/60 p-6 border border-on-surface/5 shadow-level-2 flex flex-col md:flex-row justify-between items-stretch md:items-center gap-4">
                          <div className="flex items-center gap-4">
                            ...
                            <div><span className="font-sans text-[10px] uppercase tracking-widest text-terracotta-earth font-bold block">COLLECTOR LEDGER IN FORCE</span><h2 className="font-serif text-lg md:text-xl text-ebony-deep font-semibold">Active Session: Roche-Gaston Consolidated Trust</h2></div>
                          </div>
                          <div className="flex items-center gap-3">
                            <span className="text-[10px] uppercase font-bold text-emerald-600 tracking-wider flex items-center gap-1.5 bg-emerald-50 border border-emerald-300 px-3 py-1.5 font-sans"><span className="w-2 h-2 rounded-full bg-emerald-600 animate-pulse inline-block" />Encrypted Session Connected</span>
                            <button onClick={() => { setIsAuthenticated(false); setAccessCode(""); }} className="border border-on-surface/15 text-xs text-on-surface hover:text-gold-leaf px-4 py-2 font-sans uppercase font-semibold bg-parchment-ivory">Lock Vault</button>
                          </div>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                          <div className="bg-surface-container-lowest p-6 border border-on-surface/5 shadow-level-2"><span className="text-[10px] uppercase tracking-wider text-on-surface-variant/50 font-sans block mb-1">TOTAL ACQUIRED VOLUME</span><div className="font-serif text-3xl font-bold text-ebony-deep">$10,850,000</div><p className="text-[11px] text-on-surface-variant/50 mt-1.5 font-sans">Across 4 Cleared Antiquities</p></div>
                          <div className="bg-surface-container-lowest p-6 border border-on-surface/5 shadow-level-2"><span className="text-[10px] uppercase tracking-wider text-on-surface-variant/50 font-sans block mb-1">COMPOUND APPRECIATION</span><div className="font-serif text-3xl font-bold text-terracotta-earth">+16.4% YoY</div><p className="text-[11px] text-on-surface-variant/50 mt-1.5 font-sans">Average annual index return</p></div>
                          <div className="bg-surface-container-lowest p-6 border border-on-surface/5 shadow-level-2"><span className="text-[10px] uppercase tracking-wider text-on-surface-variant/50 font-sans block mb-1">FREEPORT SECURITY CELL</span><div className="font-serif text-3xl font-bold text-ebony-deep">Zurich, C14B</div><p className="text-[11px] text-terracotta-earth font-bold mt-1.5 font-sans">Continuous Biometric Custody</p></div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8 lg:gap-12 items-start">
                          <div className="lg:col-span-4 bg-surface-container-lowest p-6 border border-on-surface/5 shadow-level-2 space-y-6">
                            <h3 className="font-sans text-xs uppercase tracking-widest text-terracotta-earth font-bold mb-2 pb-2 border-b border-on-surface/10">Sovereign Vault Document Library</h3>
                            <div className="space-y-4">
                              {["Certificates of Authenticity", "Lloyd's Transit Insurance Policy", "Sovereign Escrow Protocols"].map((doc, i) => (
                                <div key={i} className="flex justify-between items-center bg-surface-container/40 p-3.5 border-l-2 border-gold-leaf font-sans text-xs">
                                  <div><span className="font-bold text-ebony-deep block">{doc}</span><span className="text-on-surface-variant/50 text-[10px]">PDF (Signed)</span></div>
                            ...
                            <div className="p-4 bg-surface-container-low border border-on-surface/10 text-xs font-sans space-y-2">
                              <span className="font-bold uppercase tracking-wider block text-[10px] text-ebony-deep flex items-center gap-1"><Building className="w-3.5 h-3.5 text-gold-leaf" /> Swiss Freeport Trustee</span>
                              <p className="text-on-surface-variant/60 leading-relaxed text-[11px]">Your assets are secured inside the highly regulated Geneva Freeports under fiduciary care.</p>
                            </div>
                          </div>
                          ...
                          <div className="lg:col-span-8 bg-ebony-deep text-parchment-ivory p-6 md:p-8 border border-gold-leaf/25 shadow-2xl">
                            <div className="flex justify-between items-center border-b border-parchment-ivory/10 pb-4 mb-6">
                              <div><span className="font-sans text-[10px] uppercase tracking-widest text-terracotta-earth font-bold block">Firewall: active</span><h3 className="font-serif text-xl text-gold-leaf mt-0.5">Encrypted Curatorial Channel</h3></div>
                              <div className="text-right flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse block" /><span className="font-mono text-[10px] uppercase text-parchment-ivory/40">AES-256 Enabled</span></div>
                            </div>
                            <div className="h-80 overflow-y-auto mb-6 space-y-4 font-sans pr-2">
                              ...
                              {isTyping && <div className="text-xs text-parchment-ivory/40 italic font-mono flex items-center gap-2"><span className="w-2 h-2 bg-gold-leaf rounded-full animate-bounce" />Curator is querying physical vaults...</div>}
                            </div>
                            <form onSubmit={handleSendMessage} className="flex gap-4">
                              <input type="text" placeholder="Ask Coordinator..." value={typedMessage} onChange={(e) => setTypedMessage(e.target.value)} className="flex-1 bg-neutral-900 border border-neutral-800 text-sm py-3 px-4 font-sans text-parchment-ivory placeholder-neutral-500 focus:outline-none focus:border-gold-leaf" />
        ...
        {selectedPackage && (
          <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.96, y: 15 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.96, y: 15 }} className="bg-parchment-ivory border border-on-surface/10 max-w-4xl w-full p-6 md:p-10 relative z-10 shadow-2xl overflow-y-auto max-h-[90vh]">
              <div className="flex justify-between items-start border-b border-on-surface/15 pb-4 mb-6">
                <div><span className="font-sans text-[10px] uppercase tracking-widest text-gold-leaf font-bold">ADVISORY INTEGRITY REPORT</span><h3 className="font-serif text-2xl md:text-3xl text-ebony-deep mt-1">{selectedPackage.title}</h3></div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-8 text-sm">
                <div className="md:col-span-7 space-y-6">
                  ...
                  <div><h4 className="font-sans text-xs uppercase tracking-widest text-gold-leaf font-bold mb-3">Portfolio Key Metrics</h4><div className="grid grid-cols-2 gap-4"><div className="bg-surface-container p-4"><span className="text-[10px] uppercase tracking-wider text-on-surface-variant/50 font-sans block">TOTAL ACQUISITION</span><span className="font-serif text-xl font-bold text-ebony-deep">{selectedPackage.estimatedValue}</span></div><div className="bg-surface-container p-4"><span className="text-[10px] uppercase tracking-wider text-on-surface-variant/50 font-sans block">COMPOUND RETURN</span><span className="font-serif text-xl font-bold text-terracotta-earth">{selectedPackage.appreciationRate}</span></div></div></div>
                  <div><h4 className="font-sans text-xs uppercase tracking-widest text-gold-leaf font-bold mb-3">Target Allocations</h4><span className="font-serif text-on-surface block py-2 border-l-2 border-gold-leaf pl-4 bg-surface-container/30">{selectedPackage.allocation}</span></div>
                  <div><h4 className="font-sans text-xs uppercase tracking-widest text-gold-leaf font-bold mb-3">Included Artifacts</h4><div className="space-y-3">{selectedPackage.artifacts.map((art) => <div key={art.id} onClick={() => setSelectedArtifact(art)} className="flex items-center gap-4 bg-surface-container hover:bg-surface-container-low p-3 transition-colors cursor-pointer border-l-4 border-terracotta-earth"><img src={art.imageUrl} referrerPolicy="no-referrer" className="w-12 h-12 object-cover" alt="" /><div className="flex-1"><span className="font-serif text-sm font-bold text-ebony-deep block">{art.title}</span><span className="text-[11px] text-on-surface-variant/50 font-sans">{art.period}</span></div><ChevronRight className="w-4 h-4 text-on-surface-variant/60" /></div>)}</div></div>
                </div>
                <div className="md:col-span-5 space-y-6">
                  <div className="bg-surface-container p-5 border border-on-surface/5">
                    <h4 className="font-sans text-[11px] uppercase tracking-wider text-ebony-deep font-bold mb-3 border-b border-on-surface/10 pb-2">SWISS TRUSTEE STORAGE</h4>
                    <p className="text-xs text-on-surface-variant/80 leading-relaxed mb-4">This package qualifies for continuous secure storage under Swiss freeport legislation.</p>
                    <button onClick={() => { setSelectedPackage(null); formRef.current?.scrollIntoView({ behavior: "smooth" }); }} className="bg-ebony-deep w-full py-3.5 text-parchment-ivory font-sans text-xs uppercase tracking-widest hover:bg-gold-leaf hover:text-ebony-deep transition-colors">Hold Portfolio Consultation</button>
                  </div>
                  <div className="bg-surface-container p-5 border border-on-surface/5"><h4 className="font-sans text-[11px] uppercase tracking-wider text-ebony-deep font-bold border-b border-on-surface/10 pb-2">REGULATORY DISCLOSURE</h4><p className="text-[11.5px] text-on-surface-variant/70 leading-relaxed">All artifacts are reviewed against the UNESCO 1970 Convention on cultural property.</p></div>
                </div>
              </div>
            </motion.div>
          </div>
          ...
          <div className="fixed inset-0 z-[100] overflow-y-auto flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-parchment-ivory border border-on-surface/15 max-w-3xl w-full p-6 md:p-8 relative z-10 shadow-2xl max-h-[85vh] overflow-y-auto">
              <div className="flex justify-between items-start border-b border-on-surface/10 pb-4 mb-6">
                <div><span className="font-sans text-[10px] uppercase tracking-widest text-terracotta-earth font-bold">MUSEUM-GRADE DOSSIER</span><h3 className="font-serif text-2xl text-ebony-deep mt-1">{selectedArtifact.title}</h3></div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-6">
                <div className="md:col-span-5"><img src={selectedArtifact.imageUrl} referrerPolicy="no-referrer" className="w-full aspect-square object-cover" alt="" /><div className="mt-3 text-xs text-on-surface-variant/50 font-sans flex justify-between font-bold"><span>ACCESSION: {selectedArtifact.accessionNo}</span><span>EST: {selectedArtifact.estimatedValue}</span></div></div>
                <div className="md:col-span-7 space-y-4 text-xs font-sans">
                  <div><span className="text-on-surface-variant/50 block uppercase font-bold text-[10px]">Culture</span><span className="text-ebony-deep font-serif text-sm font-semibold">{selectedArtifact.origin}</span></div>
                  <div><span className="text-on-surface-variant/50 block uppercase font-bold text-[10px]">Period</span><span className="text-ebony-deep font-serif text-sm font-semibold">{selectedArtifact.period}</span></div>
                  <div><span className="text-on-surface-variant/50 block uppercase font-bold text-[10px]">Medium</span><span className="text-ebony-deep font-serif text-sm font-semibold">{selectedArtifact.medium}</span></div>
                  <div><span className="text-on-surface-variant/50 block uppercase font-bold text-[10px]">Carbon Dating</span><p className="text-on-surface-variant/80 leading-relaxed font-serif text-[12.5px] mt-1 bg-surface-container/40 p-2 border-l border-gold-leaf">{selectedArtifact.carbonDatingDetails}</p></div>
                </div>
              </div>
              <div className="space-y-4">
                <div><h4 className="font-sans text-xs uppercase tracking-widest text-terracotta-earth font-bold mb-2">Provenance Pedigree</h4><div className="space-y-1.5 font-sans text-xs">{selectedArtifact.provenance.map((prov, i) => <div key={i} className="py-1 border-b border-on-surface/5 text-on-surface-variant/80 pl-3 border-l-2 border-gold-leaf bg-surface-container/20">{prov}</div>)}</div></div>
                <div><h4 className="font-sans text-xs uppercase tracking-widest text-terracotta-earth font-bold mb-2">Exhibition Archive</h4><div className="space-y-1 text-xs text-on-surface-variant/70">{selectedArtifact.exhibitions.map((ex, i) => <div key={i} className="flex gap-2 items-center"><BookOpen className="w-3.5 h-3.5 text-gold-leaf" /><span>{ex}</span></div>)}</div></div>
              </div>
            </motion.div>
          </div>
        )}
