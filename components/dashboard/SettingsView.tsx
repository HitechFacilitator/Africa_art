  ...
  return (
    <div className="animate-fade-in text-on-surface">
      <header className="mb-10 pb-6 border-b border-ebony-deep/10">
        <h2 className="font-serif text-3xl font-medium text-ebony-deep leading-tight">System Coordinates & Profile</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-8 bg-parchment-ivory border border-ebony-deep/5 p-8 shadow-level-1">
          <form onSubmit={handleSaveChanges} className="space-y-8">
            <div className="border-b border-ebony-deep/5 pb-6">
              <h3 className="font-serif text-base font-semibold text-ebony-deep mb-4 flex items-center gap-2"><User className="w-4.5 h-4.5" /> Collector Profile Credentials</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="flex flex-col">
                  <label className="font-sans text-[10px] uppercase font-bold tracking-widest text-zinc-400 mb-1.5">Premium Registered Name</label>
                ...
                <div className="flex flex-col">
                  <label className="font-sans text-[10px] uppercase font-bold tracking-widest text-zinc-400 mb-1.5">Account Security Standard</label>
            ...
            <div className="border-b border-ebony-deep/5 pb-6">
              <h3 className="font-serif text-base font-semibold text-ebony-deep mb-4 flex items-center gap-2"><Globe className="w-4.5 h-4.5" /> System Valuation Currency</h3>
            ...
            <div className="border-b border-ebony-deep/5 pb-6">
              <h3 className="font-serif text-base font-semibold text-ebony-deep mb-4 flex items-center gap-2"><Sparkles className="w-4.5 h-4.5" /> Curatorial Interest Directories</h3>
            ...
            <div className="pb-4">
              ...
              <div className="flex flex-wrap gap-4">
                <button type="button" onClick={() => { if (theme === 'dark') onToggleTheme(); }} className={`flex items-center gap-2 px-4 py-3 border cursor-pointer text-xs font-sans ${theme === 'light' ? 'border-gold-leaf bg-white text-ebony-deep font-bold' : 'border-ebony-deep/5 text-zinc-400 bg-zinc-50/50'}`}>
                  <Sun className="w-4 h-4 text-gold-leaf" /> Museum Ivory Light (Default Premium)
                </button>
                <button type="button" onClick={() => { if (theme === 'light') onToggleTheme(); }} className={`flex items-center gap-2 px-4 py-3 border cursor-pointer text-xs font-sans ${theme === 'dark' ? 'border-gold-leaf bg-white text-gold-leaf font-bold' : 'border-ebony-deep/5 text-zinc-500 bg-zinc-50/50'}`}>
                  <Moon className="w-4 h-4 text-gold-leaf" /> Ebony Dark Deep
                </button>
              </div>
            </div>
            ...
            <button type="submit" disabled={savingStatus} className="bg-ebony-deep text-parchment-ivory font-sans text-xs font-semibold uppercase tracking-widest px-8 py-3.5 hover:opacity-90 active:scale-98 transition-all duration-300 disabled:opacity-50 cursor-pointer border-0">
              {savingStatus ? 'Synchronizing...' : 'Certify Profile Parameters'}
            </button>
          </form>
        </div>
        ...
        <div className="lg:col-span-4 space-y-8">
          <div className="bg-parchment-ivory border border-ebony-deep/5 p-8 shadow-level-1 space-y-6">
            <h3 className="font-serif text-lg font-medium text-ebony-deep border-b border-gold-leaf/20 pb-4 flex items-center gap-2"><Lock className="w-4.5 h-4.5 text-gold-leaf" /> Secure Mainframes</h3>
