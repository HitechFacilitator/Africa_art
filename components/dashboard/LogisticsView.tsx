  ...
  return (
    <div className="animate-fade-in text-on-surface">
      <header className="mb-10 pb-6 border-b border-ebony-deep/10">
        ...
        <p className="font-sans text-xs text-on-surface-variant mt-1">{lang === "fr" ? "Surveillez vos trésors patrimoniaux en temps réel. Les coursiers utilisent des chambres blindées à bouclier thermique, la télématique satellite et des escorteurs douaniers armés." : "Monitor your heritage treasures in real-time transit. Couriers utilize temperature-shielded armored chambers, satellite telematics, and armed customs escorts."}</p>
      </header>
      ...
      {shipments.length === 0 ? (
        <div className="bg-parchment-ivory border border-dashed border-ebony-deep/10 p-16 text-center select-none">
          ...
          <p className="font-sans text-xs text-zinc-400 mt-2 max-w-sm mx-auto">{lang === "fr" ? "Toutes vos acquisitions sont actuellement stationnées dans les coffres-forts du Freeport suisse ou dans les galeries de exposition privées autorisées." : "All your acquisitions are currently stationary inside Swiss Freeport safes or authorized private display galleries."}</p>
        </div>
      ) : (
              ...
              return (
                <div key={ship.id} className="bg-parchment-ivory border border-ebony-deep/5 p-8 shadow-level-1 relative">
                  <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-6 border-b border-ebony-deep/5 pb-4">
                    <div>
                      <span className="font-sans text-[9px] font-bold tracking-widest text-terracotta-earth uppercase bg-terracotta-earth/5 px-2.5 py-1">{lang === "fr" ? "Fret Aérien Haute Sécurité" : "High-Security Air Cargo"}</span>
                      <h4 className="font-serif text-xl font-medium mt-2 text-ebony-deep">{ship.artworkTitle}</h4>
                      <p className="font-sans text-xs text-zinc-400">{lang === "fr" ? "Transporteur" : "Carrier"}: {ship.carrier} • {lang === "fr" ? "Référence de Suivi" : "Tracking Reference"}: <span className="font-mono">{ship.id.toUpperCase()}</span></p>
                    </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-6 mb-8 bg-surface-container-low p-4 border border-ebony-deep/5 select-none text-xs">
                    <div>
                      <p className="text-zinc-450 uppercase font-bold text-[9px] tracking-wider mb-0.5">{lang === "fr" ? "Bouclier Atmosphérique" : "Atmospheric Shield"}</p>
                      <p className="font-sans font-medium text-ebony-deep">{lang === "fr" ? "Cabine Air Stabilisée (20.5°C / 46%)" : "Stabilized Air Cabin (20.5°C / 46%)"}</p>
                    </div>
                    <div>
                      <p className="text-zinc-450 uppercase font-bold text-[9px] tracking-wider mb-0.5">{lang === "fr" ? "Niveau de Sécurité Physique" : "Physical Security Tier"}</p>
                    ...
                    <div className="col-span-2 md:col-span-1">
                      <p className="text-zinc-450 uppercase font-bold text-[9px] tracking-wider mb-0.5">{lang === "fr" ? "Police d'Indemnisation des Risques" : "Risk Indemnity Policy"}</p>
                      <p className="font-sans font-medium text-ebony-deep">{lang === "fr" ? "Souscrite par AXA" : "AXA Underwritten"} • {ship.insuranceCoverage}</p>
                    </div>
                  </div>
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="bg-parchment-ivory border border-ebony-deep/5 p-8 shadow-level-1 space-y-6">
              ...
              <div className="space-y-4 font-sans text-xs text-zinc-500 leading-relaxed">
                <p>{lang === "fr" ? "Après la dédouanement finale, Aduna Gallery coordonne directement avec des ingénieurs spécialisés pour une installation délicate sur site." : "Upon final customs clearance, Aduna Gallery coordinates directly with physical engineering specialists to handle delicate, white-glove site installation."}</p>
                <p className="font-bold text-ebony-deep uppercase tracking-wider text-[10px]">{lang === "fr" ? "Règlements du Protocole Incluent :" : "Protocol Regulations Include:"}</p>
                <ul className="list-disc pl-4 space-y-2 text-[11px]">
                  <li>{lang === "fr" ? "Ajustement d'humidité microchambre de qualité musée" : "Museum-Grade microchamber humidity adjustment"}</li>
                  <li>{lang === "fr" ? "Audits de rayonnement lux pour prévenir le vieillissement prématuré de la pigmentation" : "Lux radiation audits to prevent early pigmentation aging"}</li>
                  <li>{lang === "fr" ? "Fixation en acier structurel Invis-Mount pour protection sismologique" : "Invis-Mount structural steel fastening for seismological protection"}</li>
                  <li>{lang === "fr" ? "Placement de gardes physiques locaux et calibration télémétrique laser de mouvement" : "Local physical guard placement and motion laser telemetry calibration"}</li>
                </ul>
                <p className="text-[11px] mt-4 italic">{lang === "fr" ? "Pour personnaliser les directives de coursier ou modifier les coffres de livraison, contactez Helena Sterling via messagerie sécurisée." : "To customize direct courier guidelines or edit final drop-off vaults, contact Helena Sterling via secure messaging."}</p>
              </div>
            </div>
          </div>
        </div>
