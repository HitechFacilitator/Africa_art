  ...
  return (
    <div className="animate-fade-in relative">
      <header className="mb-10 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6 pb-6 border-b border-ebony-deep/10">
        <div>
          <h2 className="font-serif text-3xl font-medium text-ebony-deep leading-tight">Consultations & Vetting Advisory</h2>
        ...
        <button onClick={() => setShowBookingModal(true)} className="bg-ebony-deep text-parchment-ivory font-sans text-xs font-semibold uppercase tracking-widest px-6 py-3.5 hover:opacity-90 transition-all flex items-center gap-2 cursor-pointer border-0 shadow-sm">
          <Plus className="w-4 h-4" /> Request Advisory Call
        </button>
      </header>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-8 flex flex-col gap-6">
          <h3 className="font-serif text-lg font-medium text-ebony-deep mb-2">Scheduled Private Conferences</h3>
          {consultations.length === 0 ? (
            <div className="bg-parchment-ivory border border-dashed border-ebony-deep/10 p-12 text-center select-none">
              ...
              <p className="font-serif text-lg text-ebony-deep">No Vetting Advisory Sessions Scheduled</p>
                ...
                <div className="flex flex-col justify-between items-start md:items-end gap-4 shrink-0 text-left md:text-right border-t md:border-t-0 border-ebony-deep/5 pt-4 md:pt-0">
                  ...
                  {item.status === 'Confirmed' ? (
                    <button onClick={() => alert(`Synchronizing secure premium video feed channel. Room token issued for ${item.expertName}.`)} className="bg-ebony-deep text-parchment-ivory font-sans text-[10px] font-bold uppercase tracking-widest px-4 py-2 hover:opacity-90 transition-all flex items-center gap-1.5 cursor-pointer">
                      <Video className="w-3.5 h-3.5" /> Launch Video Advisory
                    </button>
                  ) : (
        ...
        <div className="lg:col-span-4">
          <div className="bg-parchment-ivory border border-ebony-deep/5 p-8 shadow-level-1 space-y-6">
            <h3 className="font-serif text-lg font-medium text-ebony-deep border-b border-gold-leaf/20 pb-4 flex items-center gap-2"><BookmarkCheck className="w-4 h-4 text-gold-leaf" /> Academic Vetting Vow</h3>
          ...
          <div className="fixed inset-0 bg-ebony-deep/50 backdrop-blur-xs z-50 flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-parchment-ivory border border-gold-leaf w-full max-w-xl text-ebony-deep">
              <div className="px-8 py-5 border-b border-gold-leaf/20 bg-ebony-deep text-gold-leaf flex items-center justify-between sticky top-0">
                <h3 className="font-serif text-base font-semibold tracking-wide uppercase">Schedule Curator Vetting Conference</h3>
              ...
              <div className="p-8">
                <form onSubmit={handleBookingSubmit} className="space-y-6">
                  <div className="flex flex-col">
                    <label className="font-sans text-[10px] uppercase font-bold tracking-widest text-zinc-400 mb-2">Choose Specialist Expert Advisor</label>
                  ...
                  <div className="flex flex-col">
                    <label className="font-sans text-[10px] uppercase font-bold tracking-widest text-zinc-400 mb-1.5 flex items-center gap-1">Conference Focus Topic <span className="text-red-500">*</span></label>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="flex flex-col">
                      <label className="font-sans text-[10px] uppercase font-bold tracking-widest text-zinc-400 mb-1.5">Conference Date</label>
                    ...
                    <div className="flex flex-col">
                      <label className="font-sans text-[10px] uppercase font-bold tracking-widest text-zinc-400 mb-1.5">Available Time Slot</label>
                  ...
                  <div className="flex flex-col">
                    <label className="font-sans text-[10px] uppercase font-bold tracking-widest text-zinc-400 mb-1.5">Curator Preparatory Notes</label>
                  ...
                  <div className="flex justify-end gap-4 pt-6 border-t border-ebony-deep/5">
                    <button type="button" onClick={() => setShowBookingModal(false)} className="bg-transparent border border-ebony-deep/12 px-6 py-2.5 text-xs font-sans font-bold uppercase tracking-widest text-zinc-450 hover:text-ebony-deep">Bypass</button>
                    <button type="submit" className="bg-ebony-deep text-parchment-ivory px-8 py-2.5 text-xs font-sans font-bold uppercase tracking-widest hover:opacity-90 active:scale-98 transition-all">Certify Session Booking</button>
                  </div>
                </form>
              </div>
            </motion.div>
