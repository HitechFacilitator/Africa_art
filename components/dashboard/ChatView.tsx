...
function FileAttachment({ text }: { text: string }) {
    ...
    return (
      <div className="mt-1">
        <div className="flex items-center gap-2 mb-1.5">
          ...
          <span className="text-xs font-sans font-medium text-parchment-ivory/90 truncate">{filename}</span>
        </div>
        <div
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/30">
            <span className="bg-black/60 text-white text-[10px] font-sans px-2 py-1 rounded-full">Click to zoom</span>
          </div>
        </div>
      </div>
    );
          ...
          <video
            ...
            onError={(e) => {
              ...
              if (parent) {
                ...
                fallback.className = "flex items-center gap-3 p-3 text-parchment-ivory/80 hover:text-parchment-ivory transition-colors";
        ...
        <div className="rounded-xl bg-white/5 border border-white/10 p-3">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-xs font-sans font-medium text-parchment-ivory/90 truncate">{filename}</div>
              <div className="text-[10px] text-parchment-ivory/50">Voice message</div>
            </div>
          </div>
          <audio
            src={url}
            ...
            onError={(e) => {
              ...
              if (parent) {
                ...
                fallback.href = url;
                fallback.download = filename;
                fallback.className = "flex items-center gap-2 p-2 text-parchment-ivory/70 hover:text-parchment-ivory text-xs transition-colors";
  ...
  return (
    <a
      href={url}
      download={filename}
      className="mt-1 flex items-center gap-3 bg-white/5 rounded-xl border border-white/10 px-3 py-3 hover:bg-white/10 transition-colors group"
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="text-xs font-sans font-medium text-parchment-ivory/90 truncate">{filename}</div>
        <div className="text-[10px] text-parchment-ivory/50">Click to download</div>
      </div>
      <span className="text-parchment-ivory/30 group-hover:text-parchment-ivory/60 transition-colors text-sm">⬇️</span>
    </a>
  );
}
